...
Global PlayEngineEditCount.i, PlayEngineFENstr.s
Global QuickEngine_Flag.b = #False, SampleGames_Flag = #False
...
Procedure DisplayGames()
  ...
  ExportGameBtnResult = ButtonGadget(#Btn_ExportGame, #ButtonsLeftEdgeDefaultX+185, #FENBtnDefaultY, 120, 25, "ExportGame[s]")
...
Protected DefaultFileName.s, Pattern.s, PGNFilename.s, FileID.i, G1_pos.i, G2_pos.i, GameLink0.s
Protected fullversionname.s = "PGNdbkp" + version
Protected nowdate1.s, nowdate2.s, MoveString.s, Games_Export_Count_str.s
Protected j.i, k.i, year.i, month.i, day.i, hour.i, minute.i, seconds.i, Games_Export_Count.i, dash_pos.i
...
Repeat
  Games_Export_Count_str = InputRequester("PGN game(s) export", "Export this game [" + GameLink0 + "] or Range of games [" + GameLink0 + "-" + Str(GameCount-1) + "]?", GameLink0)
  dash_pos = FindString(Games_Export_Count_str,"-",1)
  If  dash_pos <= 0
    Games_Export_Count = Val(Games_Export_Count_str)
    ;PrintN("GameIndex = " + Str(GameIndex))
  Else
    Games_Export_Count = Val(Mid(Games_Export_Count_str,dash_pos+1,6))
    CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
...
If FileID
  If GameIndex >= 1
    ...
    FilePGNs(k) = result
    For j = 1 To #halfmove_max : GameScore_Plain_HalfMoves(j) = "" : Next
...
; EnableXP
; DPIAware
; Executable = PGNdbkp_20250819.app