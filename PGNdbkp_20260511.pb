...
; Rashad, and others! Thank you all for your code examples!F
...
CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
  ...
  #MiniMaxkp_20260426x_binary = "/users/kenpchess/Public/MiniMaxkp_20260426x"  
CompilerEndIf
...
CompilerIf #PB_Compiler_OS = #PB_OS_Windows
  ...
  #MiniMaxkp_20260426x_binary = "C:\PureBasic\MiniMaxkp_20260426x"  
CompilerEndIf
...
Global MiniMaxkp_path.s, opponent_move.s, previous_moveinfo.s
Global MiniMaxkp_Engine_Handle.i, windowIDm.i, minimax_movecount.i = 0
Global opponent_move_uci.s, Your_move.s, Your_move_uci.s, MiniMaxkpIsWhite_flag.b
          ...
          Case #PB_EventType_LeftClick
            Select EventGadget()
              ...
              Case #Btn_PlayMinimax
                OpenConsole()
                MiniMaxkp_path = InputRequester("Path of Your MiniMaxkp engine binary.", "Enter the full path of your MiniMaxkp engine binary [original binary in github utilities folder]: ", "/users/kenpchess/Desktop/MiniMaxkp_20260507x")
                ; nothing else yet
...
Procedure MiniMaxkp_Interaction()
  ...
  If Open_Index1_str <> ""
    ...
    For z = 1 To Len(CurrentOpening) - 4 Step 5        ; a bit tricky, parses halfmoves at character 1,6,11,16,21,26,31,36,41,46,51,56,61,66,71,76
      ...
      Else
        opponent_move_uci = OpeningMove_UCI
  ...
  Repeat
    ...
    If FindString(output,"You have checkmated") > 0
      SetGadgetText(#Info_Field, "Congratulations! You have checkmated MiniMaxkp!")
      While WindowEvent() : Wend
      If IsProgram(MiniMaxkp_Engine_Handle) And ProgramRunning(MiniMaxkp_Engine_Handle)
    ...
    If FindString(output,"move ") > 0 And FindString(output,"long algebraic move ") <= 0
      opponent_move = Mid(output,FindString(output,"move ")+5,5)
    Else
      opponent_move = ""
    EndIf
    ...
    If minimax_movecount > 0
      If MiniMaxkpIsWhite_flag = #False
        previous_moveinfo = "MiniMaxkp move was " + Str(minimax_movecount) + ". ... " + opponent_move + "  [See the board]. "
      Else
        previous_moveinfo = "MiniMaxkp move was " + Str(minimax_movecount) + ". " + opponent_move + "  [See the board]. "
      EndIf
    Else
    ...
    Repeat
      Your_move = InputRequester("Enter your long algebraic move for MiniMaxkp", previous_moveinfo + " Enter your move [i.e. e2e4] or 'go' for Black or 'quit' to stop play: ", "e2e4")
      If Your_Move <> "go"
    ...
    If Your_move <> "quit" And Your_move <> "go"
    ...
    If Your_move <> "quit" Or Your_move = "go"
      ...
      opponent_move_uci = Left(opponent_move,2) + "-" + Mid(opponent_move,3,2)
      If MiniMaxkpIsWhite_flag = #True
        minimax_movecount = minimax_movecount + 1
      EndIf
      SetGadgetText(#Info_Field, "MiniMaxkp's move was: " + opponent_move_uci)
      Move_History(minimax_movecount,2) = opponent_move_uci
      Convert_UCI_Notation(opponent_move_uci)
      PopulateMovesGadget_MiniMaxkp()
      BoardDisplay()
      While WindowEvent() : Wend
    EndIf
...
Procedure PopulateMovesGadget_MiniMaxkp()
  ...
  If Halfmovecount % 2 <> 0
    ...
    Else
      Move_Line_Text = Space(1) + Str(minimax_movecount) + "." + Space(1) + opponent_move_uci
    EndIf
     AddGadgetItem(#Move_ListIcon_Gadget, -1, Move_Line_Text)
     ;SetGadgetItemText(#Move_ListIcon_Gadget, minimax_movecount-1, Move_Line_Text, 0)
   Else
     If MiniMaxkpIsWhite_flag = #False
       SetGadgetItemText(#Move_ListIcon_Gadget, minimax_movecount-1, opponent_move_uci, 1)
     Else
...
; EnableXP
; DPIAware
; Executable = PGNdbkp_20260511x.app
; Debugger = Console
