...
Global Program_Name.s = "Minimaxkp_20260507x"
...
Declare TakeBackMove(CurrMove.w)
Declare TimerCheck()
...
Procedure SendLog(AMessage.s)
  ...
  If FileHandleLog And log_count % 5 = 0
...
;            SendLog(infoMsg)
  ...
  If LAlpha > -1 * (#MateValue - 1)
    ...
    SendLog("move " + LMove)
    SendLog("info: CM3 Node count = " + Str(NodeCount) + "  TT Entries = " + Str(TTEntryCount))
    SendLog("info: CM4 Eval = " + MoveStack(0)\Value)
...
Procedure TakeBackMove(CurrMove.w)
  Protected LFrom.w, LTo.w, LEnPassant.w, Promopiece.w, PreviousSideToMove.w
  MoveCount = MoveCount - 1
  LFrom = MoveStack(CurrMove)\FromField
  LTo = MoveStack(CurrMove)\ToField
  If Depth <= 3
    ;PrintN("TAKEBACKMOVE1:  Lfrom = " + AlgBrd(Lfrom) + "  Lto = " + AlgBrd(LTo))
    ;SendLog("TAKEBACKMOVE1:  Lfrom = " + AlgBrd(Lfrom) + "  Lto = " + AlgBrd(LTo))
  EndIf
  
    ;If AlgBrd(Lfrom) = "g8" And AlgBrd(LTo) = "f6"
     ; PrintN("TAKEBACKMOVE2**********: Here is a g8-f6 knight move!" + " Depth = " + Str(Depth))
      ;SendLog("TAKEBACKMOVE2**********: Here is a g8-f6 knight move!" + " Depth = " + Str(Depth))
    ;EndIf
...
; EnableXP
; DPIAware
; Executable = MiniMaxkp_20260507x.app