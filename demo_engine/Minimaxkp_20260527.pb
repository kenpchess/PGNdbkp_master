...
Global Program_Name.s = "Minimaxkp_20260527"
...
FileHandleLog = OpenFile(#PB_Any, MiniMaxPBkp_Logfile, #PB_File_SharedWrite | #PB_File_Append)
...
Procedure SendLog(AMessage.s)
  ...
  If FileHandleLog And (log_count % 10 = 0 Or FindString(Amessage,"checkmate") > 0 Or FindString(Amessage,"MiniMaxkp ") > 0 )
    WriteStringN(FileHandleLog, AMessage)
    ;If FindString(AMessage,"MiniMaxkp move") > 0
       ;do nothing
    ;Else
      ;PrintN("info " + AMessage)
    ;EndIf
  EndIf
...
; EnableXP
; DPIAware
; Executable = MiniMaxkp_20260527.app