...
;in reality.
;
;                                           PGNdbkp_20251225 - PGN Game And SQLite chessdb Viewer
;
;  ————————————————————————————————————————-————-----------------------------------------------------   ———————————————————----------
...
Global Stockfish_Input_Path.s, EngineA_Input_Path.s="", EngineB_Input_Path.s="", EngineC_Input_Path.s=""
Global EngineD_Input_Path.s="", EngineE_Input_Path.s="", EngineF_Input_Path.s=""
Global EngineG_Input_Path.s="", EngineH_Input_Path.s=""
...
Procedure.s PlayGame(WhiteHandle.i, WhiteName.s, BlackHandle.i, BlackName.s, OpeningPGN.s)
  ...
  Repeat
    ...
    CompilerEndIf
    AddInfoToLogFile("...PlayGame routine - FENposition_str = " + position_history(MoveNumber,turnColor))
    ...
    If ToMove = "w"
      MoveMsg = Str(MoveNumber) + "." + Space(1) + bestmove + Chr(9) + Chr(9) + tempscore + Chr(9) + Chr(9) + Chr(9)
    Else
      MoveMsg = MoveMsg + bestmove + Chr(9) + Chr(9) + Chr(9) + Space(10) + tempscore
      If Engine_Count > 2
    ...
    If BestMove = "a1a1" Or BestMove = "mate 0" Or FindString(BestMove,"(none)") > 0 Or BestMove = Move_History(MoveNumber-1,turnColor)
      ...
      AddInfoToLogFile(CurrentName + " engine returned no move or illegal move (disconnect). Disconnect flag is set." + "Bestmove = " + Bestmove)
      AddInfoToLogFile(#CRLF$)
      CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
    ...
    If BestMove = ""     ; some strange no-moves, gonna set these to drawn or loss and set the disconnect flag, temporarily
      ...
      If Val(Score_History(MoveNumber-2,turnColor)) < -100 Or FindString(Score_History(MoveNumber-1,turnColor),"mate -") > 0
        ...
        EndIf
      Else
        If MoveNumber = (Len(OpeningPGN)/10) + 1 
          Disconnect_Flag(turnColor) = #False                    ; ignore the engine opening glitches for now
        Else
          Disconnect_Flag(turnColor) = #True
        EndIf
      EndIf
    ...
    If zero_score_flag = #True And zero_counter >= NearZero_Moves_min And MoveNumber >= Draw_MoveNumber_min
      ...
      CompilerEndIf
      AddInfoTextMsg(#CRLF$)
      AddInfoTextMsg(CurrentName + " multi-fold consecutive near-zero score with " + Str(Draw_MoveNumber_min)  + " or more moves. Game result set To Draw.")
      AddInfoTextMsg(#CRLF$)
      AddInfoToLogFile(#CRLF$)
...
EndIf
If EngineE_Input_Path = ""
  EngineE_Input_Path = #Engine5Path
  Engine_Path(5) = #Engine5Path
Else
  Engine_Path(5) = EngineE_Input_Path
EndIf
If EngineF_Input_Path = ""
  EngineF_Input_Path = #Engine6Path
  Engine_Path(6) = #Engine6Path
Else
  Engine_Path(6) = EngineF_Input_Path
EndIf
If EngineG_Input_Path = ""
  EngineG_Input_Path = #Engine7Path
  Engine_Path(7) = #Engine7Path
Else
  Engine_Path(7) = EngineG_Input_Path
EndIf
If EngineH_Input_Path = ""
  EngineH_Input_Path = #Engine8Path
  Engine_Path(8) = #Engine8Path
Else
  Engine_Path(8) = EngineH_Input_Path
EndIf
...
MatchInfoResult = ListIconGadget(#InfoMatch_Field, 50, 85, 650, 425, Space(75)+"Engine Match Information",900, #PB_ListIcon_FullRowSelect)
...
TourneyResultsResult = ListIconGadget(#TourneyResults_Gadget, 50, 560, 650, 380, Space(75)+"Engine Match Results - Sorted",900, #PB_ListIcon_FullRowSelect)
...
For zz = 1 To Engine_Count
  AddInfoTextMsg("MatchEngine" + Chr(64+zz) + " = " + Engine_Path(zz))
...
;For kk = 1 To Engine_Count            ( just start the two game engines for each game, below)
...
Procedure Print_Current_Match_Standings()
  ...
  For zz = 1 To Engine_Count
    ...
    AddInfoTextMsg("Current/Final Score:  Wins: " + Str(Engine_ScoreWin(zz)) + " | Draws: " + Str(Engine_ScoreDraw(zz)) + " | Losses: " + Str(Engine_ScoreLoss(zz)) + " | Disconnects: " + Str(Engine_ScoreDisconnect(zz)))
  Next
  ;AddInfoTextMsg("=========================================================================================================")
  AddInfoTextMsg(#CRLF$)
...
Procedure Print_Current_Match_Standings2()
  ...
  For zz = 1 To Engine_Count
    ...
    ;drawsf = Engine_ScoreDraw(Engine_Index)
    ;winsf = Engine_ScoreWin(Engine_Index)
    AddToResultsGadget(Engine_Name(Engine_Index) + " [" + Engine_Path(Engine_Index) + "]")
    AddToResultsGadget("Current/Final Score:  Wins: " + Str(Engine_ScoreWin(Engine_Index)) + " | Draws: " + Str(Engine_ScoreDraw(Engine_Index)) + " | Losses: " + Str(Engine_ScoreLoss(Engine_Index)) + " | Disconnects: " + Str(Engine_ScoreDisconnect(Engine_Index)) + " | Points: " + StrD(Engine_ScorePoints(Engine_Index)))
  Next zz
...
Procedure.s ReadUCIResponse_EngineMatch(Engine_Handle.i, timeout_ms = 300)
  ...
  If filtered_output <> ""
    Select Engine_Handle
      ...
      Case Engine_Handle(5)
        PrintN("EngineE_Output = " + output)
        AddInfoToLogFile("EngineE_Output = " + output)
      Case Engine_Handle(6)
        PrintN("EngineF_Output = " + output)
        AddInfoToLogFile("EngineF_Output = " + output)
      Case Engine_Handle(7)
        PrintN("EngineG_Output = " + output)
        AddInfoToLogFile("EngineG_Output = " + output)
      Case Engine_Handle(8)
        PrintN("EngineH_Output = " + output)
        AddInfoToLogFile("EngineH_Output = " + output)
    EndSelect
  EndIf
...
Procedure WaitForUCIResponseMatch(ProgramID, expectedResponse.s, timeout_ms = 3000)
  ...
  While ElapsedMilliseconds() - startTime < timeout_ms
    Select ProgramID
      ...
      Case Engine_Handle(5)
        ReadUCIResponse_EngineMatch(Engine_Handle(5),300)
      Case Engine_Handle(6)
        ReadUCIResponse_EngineMatch(Engine_Handle(6),300)
      Case Engine_Handle(7)
        ReadUCIResponse_EngineMatch(Engine_Handle(7),300)
      Case Engine_Handle(8)
        ReadUCIResponse_EngineMatch(Engine_Handle(8),300)
    EndSelect
    response$ = output
...
Procedure Start_or_Restart_GameMatch_Engines(Engine1_Path.s,Engine1_Handle.i,Engine2_Path.s,Engine2_Handle.i)
      ...
      If Engine1_Path <> "" And Engine1_Handle <= 0
          AddInfoTextMsg("...starting or restarting " + Engine1_Path + "...")
        ...
        If Engine2_Path <> "" And Engine2_Handle <= 0
          AddInfoTextMsg("...starting or restarting " + Engine2_Path + "...")
...
Procedure GetBestMoveUCIPVMatch(ProgramID, PositionCommand.s, searchTime_ms)
  ...
  While ElapsedMilliseconds() - startTime < searchTime_ms + 100 ; Add extra time for the response
    ...
    Select ProgramID
      ...
      Case Engine_Handle(5)
        ReadUCIResponse_EngineMatch(Engine_Handle(5),100)
      Case Engine_Handle(6)
        ReadUCIResponse_EngineMatch(Engine_Handle(6),100)
      Case Engine_Handle(7)
        ReadUCIResponse_EngineMatch(Engine_Handle(7),100)
      Case Engine_Handle(8)
        ReadUCIResponse_EngineMatch(Engine_Handle(8),100)
    EndSelect
        ...
        Select EventType()
          Case #PB_EventType_LeftClick
            Select EventGadget()
              ...
              Case #Btn_UpdSF
                Repeat
                  Stockfish_Input_Path = InputRequester("Match EngineA or Stockfish Path (path must exist)", "Please enter the complete file path of your EngineA or Stockfish executable (be careful of typos): ", "/usr/local/bin/stockfish17")
                  EngineA_Input_Path = Stockfish_Input_Path
                Until FileSize(EngineA_Input_Path) > 0
                Repeat
                  EngineB_Input_Path = InputRequester("Match EngineB Path (path must exist)", "Please enter the complete file path of your EngineB executable: ", "/Applications/ChessEnginesOSX/berserk_20250203")
                Until FileSize(EngineB_Input_Path) > 0
                Repeat
                  EngineC_Input_Path = InputRequester("Match EngineC Path (path must exist)", "Please enter the complete file path of your EngineC executable (if any): ", "/Applications/ChessEnginesOSX/patriciav5_kp")
                Until FileSize(EngineC_Input_Path) > 0 Or EngineC_Input_Path = ""
                If EngineC_Input_Path <> ""
                  Engine_Count = 3
                  Repeat
                    EngineD_Input_Path = InputRequester("Match EngineD Path (path must exist)", "Please enter the complete file path of your EngineD executable (if any): ", "")
                  Until FileSize(EngineD_Input_Path) > 0 Or EngineD_Input_Path = ""
                  If EngineD_Input_Path <> ""
                    Engine_Count = 4
                    Repeat
                      EngineE_Input_Path = InputRequester("Match EngineE Path (path must exist)", "Please enter the complete file path of your EngineE executable (if any): ", "")
                    Until FileSize(EngineE_Input_Path) > 0 Or EngineE_Input_Path = ""
                    If EngineE_Input_Path <> ""
                      Engine_Count = 5
                      Repeat
                        EngineF_Input_Path = InputRequester("Match EngineF Path (path must exist)", "Please enter the complete file path of your EngineF executable (if any): ", "")
                      Until FileSize(EngineF_Input_Path) > 0 Or EngineF_Input_Path = ""
                      If EngineF_Input_Path <> ""
                        Engine_Count = 6
                        Repeat
                          EngineG_Input_Path = InputRequester("Match EngineG Path (path must exist)", "Please enter the complete file path of your EngineG executable (if any): ", "")
                        Until FileSize(EngineG_Input_Path) > 0 Or EngineG_Input_Path = ""
                        If EngineG_Input_Path <> ""
                          Engine_Count = 7
                          Repeat
                          EngineH_Input_Path = InputRequester("Match EngineH Path (path must exist)", "Please enter the complete file path of your EngineH executable (if any): ", "")
                          Until FileSize(EngineH_Input_Path) > 0 Or EngineH_Input_Path = ""
                          If EngineH_Input_Path <> ""
                            Engine_Count = 8  
                          EndIf
                        EndIf
...
; EnableXP
; DPIAware
; Executable = PGNdbkp_20260104.app