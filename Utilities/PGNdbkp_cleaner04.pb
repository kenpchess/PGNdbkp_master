; PGNdbkp_cleaner01 - PureBasic chess PGN Cleaner
; Prompts the user to select a PGN file, processes it, and saves a cleaned version.
EnableExplicit

Global version.s = "PGNdbkp_Cleaner_v04"
...
Global LParen.s = "(" , RParen.s = ")", VertLine.s = "|"
...
CompilerEndIf
Declare.s RemoveCharacterRange(XString.s, StartPos.i, EndPos.i)
...
Procedure.s RemoveCharacterRange(XString.s, StartPos.i, EndPos.i)
  ; Returns string with characters from StartPos to EndPos removed
  ProcedureReturn Left(XString, StartPos - 1) + Mid(XString, EndPos + 1)
EndProcedure
  ...
  Procedure RemoveParentheticalLines()
    ...
    Protected nested_lparen_pos.i, nested_count.i
    
    parenpair_count = 0 : lparen_pos = 0 : rparen_pos = 0 : nested_count = 0
    ...
    While FindString(AllMoves,LParen,lparen_pos+1) > 0 And FindString(AllMoves,RParen,rparen_pos+1) > 0
      ...
      If lparen_pos > 0 And rparen_pos > 0 And rparen_pos > lparen_pos
        ...
        nested_lparen_pos = FindString(AllMoves,LParen,lparen_pos+1)
        While nested_lparen_pos > 0 And nested_lparen_pos < rparen_pos And nested_count <= 10
          nested_lparen_pos = FindString(AllMoves,LParen,lparen_pos+1)
          nested_count = nested_count + 1
          If nested_lparen_pos > 0 And nested_lparen_pos < rparen_pos                             ; it actually is nested
            RemoveCharacterRange(AllMoves, nested_lparen_pos, rparen_pos)
            PrintN("RemPar: AllMoves = " + AllMoves)
            nested_lparen_pos = FindString(AllMoves,LParen,1)
            rparen_pos = FindString(AllMoves,Rparen,1)
          EndIf
        Wend
        
        parenpair_count = parenpair_count + 1
        CompilerIf #PB_Compiler_OS = #PB_OS_MacOS
          PrintN("GameCount = " + Str(GameCount) + "...Found a pair of parens..." + "Count = " + Str(parenpair_count))
        CompilerEndIf
            ...
            While Not Eof(0)
              ...
              Else
                ...
                If GameDone_Flag = #False
                  AllMoves = AllMoves + Space(1) + line
                 EndIf
               EndIf
             Wend
             SetGadgetText(#gamescleaned,"Games Cleaned: ..." + Str(GameCount))
...
; EnableXP
; DPIAware
; Executable = PGNdbkp_cleaner04.app
